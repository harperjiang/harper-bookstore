	...
	public synchronized Session getSession() {
		if (null == session) {
...

			{
				...
				login.setUserName(ConfigManager.getInstance().getConfigValue(
						"JDBC_PRINCIPLE"));
				login.setPassword(ConfigManager.getInstance().getConfigValue(
						"JDBC_CREDENTIAL"));
			}

			session = proj.createServerSession(3, 3);
